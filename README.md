...
4. Available branches: `dev` and current stable release branches (e.g. `v1.1.0`)

You can switch to **supported stable releases** and back to `dev` at any time.
Retired releases (`v1.0.0`, `v1.0.1`) are not listed. Feature branches are
intentionally not listed. If a release branch is not yet checked out locally,
the app creates it automatically from the remote.
