...
8. Switch Branch - Switch between dev and stable branches (e.g. v2.0.1); local changes are stashed before the switch and restored after it
...
- `v2.0.1` - Stable release branch. This is the current stable version. Recommended for installs (documented `git checkout v2.0.1` in the README). Bug fixes only.
...
8. The stable release branch (currently `v2.0.1`) is for production use
...
- Switch between dev and stable branches from the TUI (e.g. v2.0.1)
...
Download `install-taxicheck-v2.0.1-windows.exe` from the GitHub release and
...
```sh
make build-windows-installer   # dist/install-taxicheck-v2.0.1-windows.exe
